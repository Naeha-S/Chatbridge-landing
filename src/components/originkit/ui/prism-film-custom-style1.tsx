...
const vec2 CENTRE = vec2(0.36, 0.52);
const float TILT = 2.4;
const float ZOOM = 1.05;
const float THETA = 2.12;
const float SHEAR = 0.958;
const float SHRINK = 0.95;
const vec2 WARP_FREQ = vec2(0.5, 2.7);
const vec2 WARP_AMP = vec2(0.14, 0.03);
const vec2 ASPECT = vec2(1.8, 0.2);
const float OFFSET = 0.33;
const float GLOW = 0.0021;
const float SOFT = 0.0019;
const float FALLOFF = 0.37;
const float PHASE = 40.0;
...
const float NEAR_SPLIT = ${NEAR_SPLIT.toFixed(3)};
const float SMEAR = ${SMEAR.toFixed(4)};
...
vec3 prism(vec2 frag, vec2 uv) {
  float time = uTime * 0.22;
  float w = near(frag);
  float ang = time * 0.7 + uv.y * 1.6;
  vec2 dir = vec2(cos(ang), sin(ang)) * (1.0 - 0.5 * min(length(uTilt), 1.0)) + uTilt * 1.6;
  vec2 px = dir * uSplit;

  if (w > 1e-4) px += dir * NEAR_SPLIT * w + uVel * SMEAR * w;
  vec2 split = px * uPR / uRes;
  vec3 base = scene(uv);
  vec3 refr = vec3(scene(uv + split).r, base.g, scene(uv - split).b);
  float inten = length(base) * 0.57735;
  float presence = smoothstep(0.025, 0.2, inten);
  float phase = inten * 4.0 + dot(uv, vec2(1.6, -1.2)) + time + dot(uTilt, vec2(2.4, -1.6));
  float coat = smoothstep(0.04, 0.28, inten) * (1.0 - smoothstep(0.72, 1.0, inten));
  vec3 hsv = rgb2hsv(max(refr, 0.0));
  hsv.x += sin(phase) * (0.07 + 0.1 * w) * coat;
  hsv.y += (1.0 - hsv.y) * coat * 0.22 * smoothstep(0.04, 0.16, hsv.y);
  return mix(base, hsv2rgb(hsv), presence);
}

void main() {
  vec2 frag = gl_FragCoord.xy;
  vec3 L = max(prism(frag, frag / uRes), 0.0);
...
function __OriginkitBase_PrismFilm(props: PrismFilmProps) {
    const {
        style,
    ...
    const vRef = useRef({ background, color1, color2, speed: 1, size: 1, angle: 0, split: 3, hover: 1, reach: 260 })
    vRef.current = {
        background,
        color1,
        color2,
        ...
        split: clampN(num(split, 3), 0, 40),
        hover: clampN(num(hover, 100), 0, 200) / 100,
        reach: clampN(num(reach, 260), 10, 800),
    }
    ...
    useEffect(() => {
        ...
        const un = locations(gl, finish, ["uField", "uRes", "uTime", "uBg", "uPaper", "uPR", "uMouse", "uOn", "uReach", "uVel", "uTilt", "uSplit"])
        ...
        const render = (now: number) => {
            ...
            const weight = on * v.hover
            const kt = 1 - Math.exp(-dt * TILT_RATE)
            tiltX += (clampN((mx / cw - 0.5) * 2, -1, 1) * weight - tiltX) * kt
            tiltY += (clampN((0.5 - my / ch) * 2, -1, 1) * weight - tiltY) * kt
            ...
            gl.uniform2f(un.uMouse, mx * pr, bh - my * (bh / ch))
            gl.uniform1f(un.uOn, weight)
...
export default function PrismFilm(props: Record<string, unknown>) {
  return <__OriginkitBase_PrismFilm {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}